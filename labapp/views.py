...
def home(request):
	profile_paragraphs = [list(dict(x).values())[0] for x in dict(pd.read_csv(datadir + "profile_paragraphs.txt", sep = "\n", header = None, encoding="ISO-8859-1").T).values()]
	return(render(request, 'labapp/home.html', {'profile_paragraphs' : profile_paragraphs}))
...
def software(request):
	...
	print(software)
	return(render(request, 'labapp/software.html', {'software_info' : software}))

def people(request):
	people = pd.read_csv(datadir + "people.txt", sep = "\t")
	people = people.fillna("$$$")
	people = [dict(x) for x in dict(people.T).values()]	
	return(render(request, 'labapp/people.html', {'people' : people}))
...
def publications(request):
	...
	return(render(request, 'labapp/publications.html', {'papers' : papers}))
...
def research(request):
	...
	return(render(request, 'labapp/research.html', {'research_paragraphs' : research_paragraphs}))

def contact(request):
	return(render(request, 'labapp/contact.html'))
